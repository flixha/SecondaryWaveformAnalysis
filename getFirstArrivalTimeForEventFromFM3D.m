function firstArrival = getFirstArrivalTimeForEventFromFM3D(eventC,...
    arrivals, sta, traceOrder, evID, arrivalLine)
    

    firstArrivalPhase = char(arrivals.arrivals.(sta).phase(1));
    firstArrivalPhases = firstArrivalPhase(1:3:end)';
    
    if strcmp(firstArrivalPhases,repmat('P',...
            length(firstArrivalPhases),1))
        firstArrival = arrivals.arrivals.(sta).time(1);
%     else
...
%         while jm1 > 1
%             jm1 = jm1 - 1;
%             evIDm1 = eventC.cat.table.EventID(traceOrder(jm1));
% 
...
%         jp1 = arrivalLine;
%         firstArrP1 = nan;
%         while jp1 < eventC.cat.numberOfEvents - 1
%             jp1 = jp1 + 1;
%             evIDp1 = cat.table.EventID(traceOrder(jp1));
% 
